...
get_players_dt <- function() {
  ...
  players[, `:=`(
    ...
    height_Z = (height_inches - mean(height_inches, na.rm = TRUE)) /
      sd(height_inches, na.rm = TRUE)
  )]
...
get_offense_formation <- function(tracking, plays) {
  ...
  checkmate::check_character(
    unique(plays[!is.na(offenseFormation), offenseFormation]),
    any.missing = FALSE
  )
  ...
  offense_formation <- unique(tracking[, c(
    'gameId',
    'playId',
    'mirrored',
    'frameId',
